...
- (id)init
{
    ...
    if (self) {
        ...
        RACSignal *worldModel = [RACObserve(self, worldModel) map:^id(id value) {
            return @"worldModelChanged";
        }];
        
        RACSignal *latest = [RACSignal combineLatest:@[worldModel, viewDidLoad] reduce:^id{
            return @"latest";
        }];
...
-(void)setUpListeners:(id)sender{
    ...
    self.title = self.worldModel.name;
    ...
    messageAddedQuery = [[[fireService worldMessagesRef:self.worldModel.worldId] queryOrderedByChild:@"timestamp"] queryLimitedToLast:300];
    ...
    [[[fireService worldMessagesRef:self.worldModel.worldId] queryLimitedToLast:1] observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot) {
        [self.tableView reloadData];
        initialAdds = NO;
    }];
    ...
    worldShoutsRef = [fireService worldShoutsRef:self.worldModel.worldId];
    ...
    worldPushBusRef = [fireService worldPushBusRef:self.worldModel.worldId];
    ...
    messageRemovedQuery = [fireService worldMessagesRef:self.worldModel.worldId];
...
-(void)pageMoreMessages:(NSDate *)timestamp{
    ...
    [[[[[fireService worldMessagesRef:self.worldModel.worldId] queryOrderedByChild:@"timestamp"] queryStartingAtValue:timestampNumber] queryLimitedToLast:30] observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot) {
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        MessageModel *messageModel = [self.messages objectAtIndex:indexPath.row];
        [[[fireService worldMessagesRef:self.worldModel.worldId] childByAppendingPath:messageModel.messageId] removeValue];
    }
}
...
-(void)didPressRightButton:(id)sender{
    ...
    [[fireService submitWorldMessage:self.worldModel.worldId worldName:self.worldModel.name myUserId:userModel.userId mySpriteUrl:userModel.spriteUrl myName:userModel.name myUserImageUrl:userModel.imageUrl text:messageText imageUrl:@""] subscribeNext:^(id x) {
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [[[uploadService uploadImage:chosenImage] flattenMap:^RACStream *(id value) {
        ...
        UserModel *userModel = [NSUserDefaults standardUserDefaults].userModel;
        return [fireService submitWorldMessage:self.worldModel.worldId worldName:self.worldModel.name myUserId:userModel.userId mySpriteUrl:userModel.spriteUrl myName:userModel.name myUserImageUrl:userModel.imageUrl text:@"" imageUrl:value];
    }] subscribeNext:^(id x) {
        @strongify(self);
...
-(void)worldDetailDidClick:(id)sender{
    ...
    detailViewController.world = self.worldModel;
