...
-(RACSignal *)submitWorldMessage:(NSString *)worldId worldName:(NSString *)worldName myUserId:(NSString *)myUserId mySpriteUrl:(NSString *)mySpriteUrl myName:(NSString *)myName myUserImageUrl:(NSString *)myUserImageUrl text:(NSString *)text imageUrl:(NSString *)imageUrl{
    ...
    RACSignal *worldPushNote = [self submitWorldPushNoteWithWorldId:worldId worldName:worldName userId:myUserId userName:myName messageText:text messageImageUrl:imageUrl];
...
-(RACSignal *)submitWorldPushNoteWithWorldId:(NSString *)worldId worldName:(NSString *)worldName userId:(NSString *)userId userName:(NSString *)userName messageText:(NSString *)messageText messageImageUrl:(NSString *)messageImageUrl{
        return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            [[[rootRef childByAppendingPath:@"worlds-push-bus"] childByAppendingPath:worldId] setValue:@{@"worldName": worldName, @"userId": userId, @"userName": userName, @"messageText": messageText, @"messageImageUrl": messageImageUrl} withCompletionBlock:^(NSError *error, Firebase *ref) {
                if (error) {
                    [subscriber sendError:error];
                }else{
                    [subscriber sendNext:nil];
